...
void MCU_Initialization(void)
{
	...
	//PT1端口[7:1]
	//-------------------------------
	PT1CON = 0x01;					//PT1CON:(上电默认值:00000000)
	...
	PT1CON1 = 0x00;					//PT1CON1(上电默认值：uuuu0000)
	...
	PT1EN = 0x30;					//PT1EN:(上电默认值:0000u00u)
	...
	PT1 = 0x00;						//PT1:(上电默认值:xxxxxxxu)	


	//-------------------------------
	//PT3端口[7:0]，设置PT3.4作为UART发送口，设置P3.0为AD模拟输入口
	//-------------------------------
	PT3CON	= 0x01;					//PT3CON:(上电默认值:00000000)
									//Bit7-0 PT3CON[7:0]: GPIO3模拟/数字端口使能标志
									//	0 = 定义为数字口
									//	1 = 定义为模拟口
	
	PT3EN = 0x65;					//PT3EN:(上电默认值:00000000)
									//	0 = 定义为输入口
									//	1 = 定义为输出口	
	...
	PT5EN = 0x01;					//PT5EN:(上电默认值:00000000)
	
	PT5PU = 0x00;					//PT5PU:(上电默认值:00000000)

	PT5 =	0x00;					//PT5:(上电默认值:xxxxxxxx)
	...
	//-----------------------------------------------------------------*/
  	TM2CON  = 0x00;		   			//TM2CON:(上电默认值为:00000100) 
...
//--------------------------------------------
	...
	SCON1 = 0x50;					//SCON1:(上电默认值位:000000uu)
									//Bit7-6 SM0 SM1: 串口通信工作方式
									//		 方式   类型   波特率
									//		00 = 0      同步   BaudRate = Fcpuclk/6
									//		01 = 1      异步   
									//						   TM4溢出率/16(SMOD=1)
									//						   TM4溢出率/32(SMOD=0)
									//		10 = 2		异步   Fcpuclk/16(SMOD=0)
									//						   Fcpuclk/32(SMOD=1)
									//		11 = 3		异步  
									//					   		TM4溢出率/16(SMOD=1)
									//					   		TM4溢出率/32(SMOD=0)
	...
	SCON2 = 0xef;					//SCON2:(上电默认值:000u0000)，设置波特率为9600
									//Bit 7   SMOD: 波特率选择寄存器
									//		0：定时器0/2/3/4的溢出率32
									//		1：定时器0/2/3/4的溢出率16
									
									//bit[6:5]	URCLK[1:0]	UART时钟选择位              
									//			URCLK [1:0]		UART时钟                
									//          00   		选择定时器0溢出作为时钟，   
									//                       SMOD=1，波特率为溢出率/16，
									//                       SMOD=0，波特率为溢出率/32  
									...
									//bit 3		UARTEN		UART模块使能                
									//         	0：不使能UART模块                           
									//          1：使能UART模块                             
									//          若关闭UART模块使能位，则不会产生中断标志位。
									
									//bit[2:0]  PINCTL[2:0] UART通信口选择                                                                 
									//          3’b000      PT1.1、PT1.2做普通IO口，PT3.3、PT3.4做普通IO口                                                
									//          3’b001      PT1.1做UART通信口，方向由PT1EN[1]决定，PT1.2做普通IO口，PT3.3、PT3.4做普通IO口                                                                                                                            
									//          3’b010      PT1.2做UART通信口，方向由PT1EN[2]决定，PT1.1做普通IO口，PT3.3、PT3.4做普通IO口                                              
									//          3’b011      PT1.1、PT1.2分别做UART通信口RI、TI， PT3.3、PT3.4做普通IO口                       
									//          3’b100      PT1.1、PT1.2做普通IO口，PT3.3、PT3.4做普通IO口                                    
									//          3’b101      PT3.3做UART通信口，方向由PT3EN[3]决定，PT3.4做普通IO口，PT1.1、PT1.2做普通IO口    
									//          3’b110      PT3.4做UART通信口，方向由PT3EN[4]决定，PT3.3做普通IO口，PT1.1、PT1.2做普通IO口    
									//          3’b111      PT3.3、PT3.4分别做UART通信口RI、TI， PT1.1、PT1.2做普通IO口                       
									//          	当选择同步通信或者同时需要进行发送和接收时，必须有两个口配置为通信口，                   
									//          	即PINCTL[2:0]必须配置为3’bx11。当PINCTL[2:0]配置为3’bx01或3’bx10时，                  
									//          	此时只有一个通信口在使用，此时这个通信口可以分时复用为RI和TI，                           
									//          	方向有IO口的方向位控制，例如：当把PINCTL[2:0]配置为3’b001时，                           
									//          	此时只有PT1.1是UART通信口，当PT1EN[1]为0时，PT1做为RI口，当PT1EN[1]为1时，PT1做为TI口。										
